...
class RecordList extends Component {
  ...
  delete=(id)=>{
   ...
   plus=()=>{
     let amount=this.state.records.filter(item=>{
       return item.amount >=0;

     })
     return amount.reduce((prev,cur)=>{

        return prev+Number.parseInt(cur.amount,0);
     },0)
   }
    pay=()=>{
     let payed=this.state.records.filter(item=>{
       return item.amount <0

       }
      )
      return payed.reduce((prev,cur)=>{
        return prev+Number.parseInt(cur.amount,0)
      },0)
    ...
    return (
      <div>
        ...
        <div className="row mb-3">
           <AmountBox  text="Credits" type="success"  computed={this.plus}></AmountBox>
           <AmountBox text="Debits" type="danger" computed={this.pay}></AmountBox>
           <AmountBox text="Balance" type="info" computed={this.rest}></AmountBox>
        </div>
